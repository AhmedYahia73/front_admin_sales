...
const LeaderAdd = () => {
    ...
    const { data: listsResponse, loading: isLoadingLists } = useGet("/api/admin/leader/lists");
    const targetList = listsResponse?.data?.target_list || listsResponse?.target_list || [];

    const { data: response, loading: isLoadingData } = useGet(
        id ? `/api/admin/leader/${id}` : null,
        Boolean(id)
    );

    const fetchedLeader =
        response?.data?.leader?.[0] ||
        response?.leaders?.[0] ||
                                        ...
                                        render={({ field }) => (
                                            ...
                                            >
                                                <SelectTrigger className="h-10 text-sm rounded-md">
                                                    <SelectValue placeholder={isLoadingLists ? "Loading targets..." : "Select target"} />
                                                </SelectTrigger>
                                                <SelectContent>
                                                    {targetList.map((targetItem) => (
                                                        <SelectItem key={targetItem.id} value={String(targetItem.id)}>
                                                            {targetItem.name}
                                                        </SelectItem>
                                                    ))}
                                                </SelectContent>
